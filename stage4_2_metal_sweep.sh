...
echo "=================================================="
echo "🔍 Phase 4-2: sweep metal_thr (Above base score 0.010)"
echo "Fixed: default_thr=0.010, cardboard_thr=0.0, metal_nms_iou=0.72"
echo "=================================================="

for MTHR in 0.012 0.015 0.018 0.022 0.025; do
  TAG="metal_${MTHR//./}_d0010"
  OUT_JSON="$OUT_DIR/post_${TAG}.json"
  
  echo "--------------------------------------------------"
  echo "⚙️ Running metal_thr = $MTHR"
  python "$POST_SCRIPT" \
    --gt_json "$GT_JSON" \
    --pred_json "$RAW_JSON" \
    --out_json "$OUT_JSON" \
...
echo "=================================================="
echo "⏸️ Phase 4-2 完毕！请向上滚动查看 mAP 结果！"
echo "=================================================="
